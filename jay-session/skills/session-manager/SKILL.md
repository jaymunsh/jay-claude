...
`.claude/session/` 을 처음 만들 때 `.gitignore` 에 항목이 없으면 **한 번만** 묻는다. 기본은 등록(개인 작업 흐름이라 커밋 로그와 중복되고, devlog가 매 턴 커져 diff를 오염시킨다)이지만, 팀과 인계 문서를 공유할 목적이면 커밋하는 게 맞다. 이미 물어봤거나 항목이 있으면 다시 묻지 않는다. **git 저장소가 아니면 이 질문 자체를 하지 않는다** — 커밋할 저장소가 없는데 무엇을 무시할지 물으면, 없던 `.gitignore` 를 새로 만들게 된다.
