...
2. 아직 꺼져 있으면 `AskUserQuestion` 으로 묻는다:
   ...
   - **git 저장소이면서** `.gitignore` 에 `.claude/session/` 이 아직 없다면 추가할지도 같이 묻는다. git 저장소가 아니면 이 질문은 통째로 뺀다 — 없던 `.gitignore` 를 만들 뿐이다. 기본은 추가(개인 작업 흐름이라 커밋 로그와 중복되고, devlog가 매 턴 커져 diff를 오염시킨다). 팀과 인계 문서를 공유할 목적이면 빼는 게 맞다.
